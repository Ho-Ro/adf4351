...
# requires the new "experimental" fx2 firmware
...
eeprom = intf.get_eeprom()

if eeprom[-2] != 0xEC: # EEPROM checksum magic value
    print( 'no valid default reg set')
    print( eeprom )
    sys.exit(1)

cs_ee = eeprom[-1] # checksum from eeprom
...
if  cs_ee == cs_calc: # cs match
    regs = struct.unpack( '6I', eeprom[0:24] ) # get 6 register values
